...
from fall_detector_npu import process_video_npu
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="Fall detection with RKNN NPU acceleration (RK3588)")
    parser.add_argument(
        "--imgsz",
        type=int,
        default=640,
        help="Inference size (default 640, supports 320/480/640/1280 with corresponding RKNN models).",
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    out_name = f"{base_name}_fall_detected_npu.avi"

    start = time.time()
    out_path = process_video_npu(
        inp,
        weights_dir="weights",
        results_dir="results",
    ...
    dur = time.time() - start
    logger.info(f"Saved NPU result to: {out_path} (processed in {dur:.2f}s)")
