...
import cv2
import numpy as np
from rknn_pose_model import RKNNPoseModel
from rknn_seg_model import RKNNSegModel
...
def locate_rknn_model(weights_dir: str, model_type: str, imgsz: int) -> str:
    """Locate RKNN model file."""
    model_name = f"yolo11n-{model_type}-{imgsz}.rknn"
    model_path = os.path.join(weights_dir, model_name)
    if os.path.exists(model_path):
        return model_path
    default_name = f"yolo11n-{model_type}.rknn"
    default_path = os.path.join(weights_dir, default_name)
    if os.path.exists(default_path):
        if imgsz != 640:
            logging.getLogger("fall_detection").warning(
                f"Model for {imgsz} not found, using default 640: {default_path}"
            )
        return default_path
    raise FileNotFoundError(
        f"RKNN {model_type} model not found. Place {default_name} in {weights_dir}/"
    )


def process_video_npu(
    ...
    pose_model_path = locate_rknn_model(weights_dir, "pose", imgsz)
    pose_model = RKNNPoseModel(pose_model_path, imgsz)
    logger.info(f"Loaded RKNN pose model: {pose_model_path}")

    # Load segmentation model once (for bed detection)
    seg_model = None
    try:
        seg_model_path = locate_rknn_model(weights_dir, "seg", imgsz)
        seg_model = RKNNSegModel(seg_model_path, imgsz)
        logger.info(f"Loaded RKNN seg model: {seg_model_path}")
    ...
    try:
        ...
        while True:
            ...
            if frame_index == 0 and seg_model is not None and bed_mask is None:
                try:
                    bed_mask = seg_model.inference(frame, conf_thres=0.3, iou_thres=0.45)
            ...
            bed_bbox_current = bed_bbox

            # Overlay bed mask if requested
            if bed_mask is not None and visual_mask:
                mask_overlay = cv2.applyColorMap((bed_mask * 255).astype(np.uint8), cv2.COLORMAP_JET)
                frame = cv2.addWeighted(frame, 0.75, mask_overlay, 0.25, 0)
            ...
            # RKNN inference
            pred = pose_model.inference(x)
            boxes, scores, kps = pose_model.postprocess(pred, conf_thres=conf, iou_thres=iou)

            if len(boxes) == 0:
                writer.write(frame)
                if log_every > 0 and (frame_index % max(1, log_every) == 0):
                    logger.debug(f"frame={frame_index}: no detections")
